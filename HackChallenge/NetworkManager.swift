...
class NetworkManager {
    ...
    static func addFoodToUser(food: Food, amount: Int, date: Date, completion: @escaping (UserFood) -> Void) {
        ...
        AF.request(endpoint, method: .post, parameters: params, encoding: JSONEncoding.default).validate(statusCode: 200..<600).responseData { response in
            switch response.result {
            case .success(let data):
                let jsonDecoder = JSONDecoder()
                jsonDecoder.keyDecodingStrategy = .convertFromSnakeCase
    ...
    static func getFoodUserByDate(userId: Int, completion: @escaping([Food]) -> Void) {
